...
export default class Help extends Command {
  ...
  async exec(
    ...
    if (!category || !categories.find((it) => it.name === category)) {
      ...
      if (!command || !this.handler.findCategory(c.id).find(r=>r.id===command || r.aliases.includes(command))) {
        ...
        embed.setFooter('더 자세히 알아보려면 ]도움말 <카테고리> <명령어>를 입력해주세요')
      } else {
          const cmd = this.handler.findCategory(c.id).find(r=>r.id===command || r.aliases.includes(command))!
          embed.setTitle(`${c.name} - ${cmd.aliases[0]}`)
          embed.setDescription(cmd.description || '설명 없음')
          embed.addField('별칭', cmd.aliases.map(it => '`' + it + '`').join(', '), true)
      }
    }
